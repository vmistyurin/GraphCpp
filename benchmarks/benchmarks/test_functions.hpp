...
namespace graphcpp::bench
{
    ...
    template<class RandomGraphType, class GraphType = NonOrientedMatrixGraph<SingleVectorSymmetricMatrix>, class MatrixType = SingleVectorSymmetricMatrix>
    std::function<std::string(std::ifstream&&)> multi_threaded_matrix_of_flows(single_flow_function base_function)
    {
        IS_NOR_RANDOM_GRAPH_IMPL(RandomGraphType);
        IS_NOR_GRAPH_IMPL(GraphType);
        IS_SYM_MATRIX_IMPL(MatrixType);

        // return multi_threaded_matrix_of_flows<MatrixType, GraphType, RandomGraphType>();

        std::function<std::string(std::ifstream&&)> res = [base_function = std::move(base_function)](std::ifstream&& input)
        {
            auto graph = std::make_unique<RandomGraphType>(RandomGraphType::read_from_stream(input));

            auto calculator = graphcpp::MultiThreadCalculator<MatrixType, GraphType>(std::move(graph), [base_function = std::move(base_function)](const GraphType& graph)
            {
                return graphcpp::flow_calculators::matrix_of_flows<MatrixType, GraphType>(graph, base_function);
            });
    ...
    template<class MatrixType, class GraphType, class RandomGraphType = RandomNonOrientedGraph<GraphType, MatrixType>>
    std::function<std::string(std::ifstream&&)> multi_threaded_matrix_of_flows(std::function<MatrixType(const GraphType&, flow_calculators::ReductionStats*)> base_function)
    {
        IS_NOR_RANDOM_GRAPH_IMPL(RandomGraphType);
        IS_NOR_GRAPH_IMPL(GraphType);
        IS_SYM_MATRIX_IMPL(MatrixType);
