...
using namespace graphcpp_testing;
	...
	TEST(HelpersTests, CompareVectorsOfIntWithoutOrder)
	{
		...
		EXPECT_TRUE(compare_without_order(test_vector, test_vector));
		EXPECT_TRUE(compare_without_order(test_vector, permutated_test_vector));
		EXPECT_FALSE(compare_without_order(test_vector, different_size));
		EXPECT_FALSE(compare_without_order(test_vector, different_content));
	}
	...
	TEST(HelpersTests, CompareVectorsOfEdgesWithoutOrder)
	{
		...
		EXPECT_TRUE(compare_without_order(test_vector, equal_to_test_vector));
		EXPECT_FALSE(compare_without_order(test_vector, different_weight));
		EXPECT_FALSE(compare_without_order(test_vector, different_node));
	...
	TEST(HelpersTests, CompareListsWithoutOrder)
	{
		...
		EXPECT_TRUE(compare_without_order(test_list, test_list));
		EXPECT_TRUE(compare_without_order(test_list, permutated_test_list));
		EXPECT_FALSE(compare_without_order(test_list, different_size));
