...
{
	template<class RandomNorGraphType>
	class RandomNonOrientedGraphBaseTests : public ::testing::Test
	{
	protected:
		RandomNonOrientedGraphBaseTests() :
			test_graph(random_non_oriented_test_graph::get_graph<RandomNorGraphType>())
		{
		}
	...
	TYPED_TEST(RandomNonOrientedGraphBaseTests, GetEdgesTest)
	{
		...
		EXPECT_TRUE(compare_without_order(edges, random_non_oriented_test_graph::edges()));
	}
	...
	TYPED_TEST(RandomNonOrientedGraphBaseTests, ReadFromStreamTest)
	{
		...
		EXPECT_TRUE(compare_without_order(this->test_graph->get_edges(), graph.get_edges()));
	}
}